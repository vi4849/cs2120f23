...
-- #3 (α ⊕ β → Empty) → ((α → Empty) × (β → Empty)) 
def e2 := ¬(A ∨ B) ⇒ (¬A ∧ ¬B)

-- #4 ((α → Empty) × (β → Empty)) → ((α ⊕ β) → Empty)  
def e3 := (¬A ∧ ¬B) ⇒ ¬(A ∨ B)
...
-- Answer here
#eval eval_expr e0 (fun (j: var) => if j.n ==1 then true else false) -- expect true
#eval eval_expr e0 (fun (c: var) => if c.n ==2 then true else false) -- expect true

#eval eval_expr e1 (fun (a: var) => if a.n ==3 then true else false) -- expect true
#eval eval_expr e1 (fun (b: var) => if b.n ==0 then true else false) -- expect true

#eval eval_expr e2 (fun (a: var) => if a.n ==3 then true else false) -- expect true
#eval eval_expr e2 (fun (b: var) => if b.n ==0 then true else false) -- expect true

#eval eval_expr e3 (fun (a: var) => if a.n ==3 then true else false) -- expect true
#eval eval_expr e3 (fun (b: var) => if b.n ==0 then true else false) -- expect true

/- 
NOTES:
def assign_bool :  var → Bool
| a => if a.n == 3 then true else if a.n == 1 then true else false 
-/
