...
/-
def glue_funs : {a: Type→Type} → {b: Type→Type} → {c: Type} → Bool 
|g, f, s => g(f(s))
FAILED ATTEMPT
-/

/-
def glue_funs : {n b s : Type}  → (n→b) → (s→t) → s → Bool
|_,_,_,g, f, s => g(f(s))
FAILED ATTEMPT
-/
...
#eval glue_funs double square 5 --expect 50
#eval glue_funs square double 5 --expect 100
