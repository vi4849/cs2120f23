...
def not_either_not_both { jam cheese } :
  ((no jam) ⊕ (no cheese)) → 
  (no (jam × cheese)) 
| Sum.inl nojam => (fun (j,c) => nomatch (nojam j))
| Sum.inr nocheese => (fun (j,c) => nomatch (nocheese c))

/-
| Sum.inl nojam => (fun s => nomatch (nojam s.1))
| Sum.inr nocheese => (fun s => nomatch (nocheese s.2))
-/

/-
NOTES
- whenever you have ⊕, you need to do Sum.inl a (a = left side) and Sum.inr b to isolate the individual components
- because you are working with ⊕, either a OR b can be true, so you must provide an explanation for what will happen is
a is true, and what will happen if b is true
- when the value you return contains →, that means you are returning a function, so you need to use fun
- if you do an arrow to fun a, a will be the left side / input of the func., and you're responsible for getting the output
- nomatch takes a type Empty --> nomatch (Empty)
-/
...
| (Sum.inl noa) => (fun s => nomatch(noa s.1))
| (Sum.inr nob) => (fun s => nomatch(nob s.2))
...
-- create a, put in the left part of noaorb, which returns empty
-- has to be fun a first, fun b second
/-
NOTES
you don't need to use Sum.inl and Sum.inr for this problem because you're returning a product type (_,_)
the thing you're returning has →'s in it, so you need to use two fun
assuming a is true, put it in the left of the ⊕ function, which returns empty  



match s with 
  | Sum.inl noa => (nomatch (noa), nomatch())
  | Sum.inr nob => (nomatch (nob), nomatch())
-/
...
| noaandb => fun a => match a with 
  | Sum.inl a => noaandb.fst (a)
  | Sum.inr b => noaandb.snd (b)

-- need to take the first part, put into the left part of sum, 
/-
Similar to demorgan1 and demorgan2 (is demorgan2 but flipped)
can make noaandb (noa, nob)
-/
...
#reduce sum_f square 3 --expect 14
#reduce sum_f square 1 --expect 1
#reduce sum_f square 0 --expect 0
