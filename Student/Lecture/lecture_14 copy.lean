...
/-
NOTES
List.map: maps a function to a List and returns a new List
List.foldr: takes a binary operator and puts it in between all elements in a list -> returns one value 
List.filter: takes a function (that returns Bool) and a List and returns a List that contains the true elements
-/
...
--NOTE: map takes a List and returns a List through recursion
...
--NOTE: applies operation to entire list
--foldr with a special widget will add lengths of strings in a list
--use recursion to add up the lengths
...
--1 and 0 are the base cases (what happens when you get to the empty list[] after going through
-- [1,2,3,4,5])
...
--^first recursively evaluate the value of e1, then e2, and then the binary operator
...
--GO OVER AND UNDERSTAND
-- Mikhail
def make_bool_lists: Nat → List (List Bool)
| 0 => [[]]
| n + 1 =>  (List.map (fun L => false::L) (make_bool_lists n)) ++ --L is of type List Bool
            (List.map (fun L => true::L) (make_bool_lists n)) --make_bool_lists n contains the smaller truth tables

#eval make_bool_lists 2
#eval make_bool_lists 1

--NOTE: if you have zero variables, there is one one interpretation, which is the empty interpretation (2^0)
--function builds a bigger truth table out of a smaller truth table
--to go from 0 to 1 -> create two copies of 0 and add true to the front of one and false to the front of the other one
...
--NOTES
--given an expression -> have a function that can generate interpretations
--want to go from interpretations to Bool 
--interp is T/F, T/T, F/F, etc.
--expr is P ∧ Q
--apply eval_expr e to an interpretation to get a Bool
--(mk_interps_vars (num_vars e)) creates a list of interpretations for e
--when using List.map change eval_expr_over_interps e to eval_expr e
...
--GO OVER -> reduce_or returns a function that takes in a List of Bools
...
--takes an expression, gets the interpretations through the number of variables, applies the expression to the variables, and checks if at least one of the outputs is true
def is_valid (e : Expr) : Bool := reduce_and (truth_table_outputs e)
def is_unsat (e : Expr) : Bool := not (is_sat e) --does not (is_sat ¬e) work?? sometime yes and sometimes no?
...
--NOTE: Can't go from Expr → Model because there might not be a model if the function is 
--unsatisfiable

def SomeInterpOrNone := Interp ⊕ Unit   -- NB: this is a *type*
--NOTE: either return a correct model / result (Interp) or an "error" message (Unit)
...
--NOTE: from none you can't infer the type -> need to make the argument explicit
--doesn't return an interpretation because some expresions might not have a model
--partial function: log(x) -> undefined for negative values, partial function for real numbers 
--functions in lean must be total -> has to be able to return a value of the return type, so you do Real -> Option 
  -- allows you to do Option.none or the answer
...
| e =>
  let interps := mk_interps_expr e --get list of interpretations
  find_model_helper interps e
where find_model_helper : List Interp → Expr → Option Interp
| [], _ => none --no interpretations found -> return none
| h::t, e => if (eval_expr e h) then some h else find_model_helper t e --keep calling function until empty
--e is the expression, h is the interpretation -> if satisfiable, return true (some h)
--returns the first interpretation that works
--you have to deconstruct Option Interp
--lean automatically opens the Option namespace
...
--NOTE: fallacy: pattern of reasoning which is not logically sound (not valid)
--counterexample: model of the negation of the formula
...
#check ((X ⇒ Y) ⇒ (¬Y ⇒ ¬X)) --IMPORTANT: this works! called the CONTRAPOSITIVE
