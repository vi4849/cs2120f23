...
-- NOTE: inductive means that you are defining a type -> assign Object to Type
-- rock, paper, and scissors are constructors
...
#check Object.rock --NOTE: a way to get the type without doing open Object
...
inductive Result : Type --NOTE: enumerated type
| wins
| loses
| ties
/-
NOTES:
define data type called Result, three ways of building this type are wins, loses, or ties
wins is an object of type result
-/
...
/-
NOTE
objects and results are type
type of win is result
type of result is type
IMPORTANT: value of type object and value of type object gives back a value of type result
-/

#eval play scissors rock --doesn't work
#reduce play scissors rock --IMPORTANT: use reduce for objects
...
| put (a : α) --NOTE: can hold things they are of type type
-- Type of Box (α : Nat) : Type is Type
...
def box_containing_hello':= Box.put "hello" --no explicit declaration of the variable type, it's ok because lean can infer
-- use a constructor of a data type by applying it to some arguments (argument never gets evaluated)

#check (box_containing_zero)
#check (Box)
#check (@Box.put) --Nat → value of Nat → Box Nat       STUDY THIS IMPORTANT NOTE
--IMPORTANT
--constructor of polymorphic type is itself polymorphic (takes things implicitly)
...
--NOTE: type of Box.put 1 is Nat → Box Nat
...
def get {α : Type} : Box α → α --NOTE: gets rid of the Box, so you directly access alpha
| (put o) => o
...
inductive Prod (α β : Type) : Type --NOTE: type of Prod is Type → Type → Type, type of Prod Nat Bool is Type
| pair (a : α) (b : β)
...
#check (@pair) -- Type → Type → value of alpha → value of b → value of type prod a b (like a box with a on one side and b on another side)
...
--NOTE: do eliminating through pattern matching
...
def first {α β : Type} : Prod α β → α --pattern matching
| (pair a _) => a
...
-- type of pair 0 is Prod Bool Nat
#check pair0

-- It's better to use standard "outfix" notation
def pair1 := ("Hello",5) --pair1 type is Prod String nat
def pair2 := (17, false)
