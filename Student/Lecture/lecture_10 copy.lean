...
inductive Doll : Type
| solid
| shell (d : Doll) -- shell with another doll inside it
--NOTE: inductive data types are finitely constructed -> can't create an infinite Doll -> the set of all types of Dolls that you can
--construct is infinite
--IMPORTANT: have an infinite set of dolls, but every doll is finitely constructed
...
-/
def d0 := solid
def d1 := shell d0 --nesting depth: 1
def d2 := shell d1
def d3 := shell d2 --nesting depth: 3
...
--NOTE: build big things out of smaller things of the same type (ex. trees)
...
| d => bad_depth' d       -- not a structural recursion, there's no base case
...
#check (bad_depth d3)           -- Term of type Empty!!! -> one should never be able to write something of type Empty
