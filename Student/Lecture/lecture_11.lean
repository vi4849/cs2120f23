...
--NOTE: Can tell if something is a proposition if you can put "is it true that" in front of it
...
/- 
NOTES
you input variables into functions and return variables 
expressions are propositions
-/

- If *a* is an atomic formula / variable then *{a}* is an expression / proposition
...
| a       |   ¬{a}    |
|---------|-----------|
| true    |  false    | --NOTE: each row represents one evaluation / interpretation
| false   |  true     |
...
| a      | b      |  {a} ∨ {b}  | --NOTES: func would take two variables and return a boolean
|--------|--------|-------------| 
| true   | true   |     true    |
| true   | false  |     true    |
| false  | true   |     true    |
...
|--------|--------|-------------|
|        i₃       |     true    | 
|        i₂       |     true    | --NOTE i2 takes two variables and assigns one to true and one to false
|        i₁       |     true    |
|        i₀       |     false   |
...
inductive Expr : Type
| var_exp (v : var) --NOTE takes a variables and turns it into an expression
| un_exp (op : unary_op) (e : Expr) --returns a bigger expression
| bin_exp (op : binary_op) (e1 e2 : Expr) --returns a bigger expression
...
def s4 := bin_exp binary_op.and s0 s3 -- is of type expression
def s5 := bin_exp binary_op.or s0 s3
def s6 := bin_exp binary_op.or s4 s5 -- NOTE: base case is a variable expression -> contains variables
...
prefix:max "¬" => un_exp unary_op.not --NOTE: prefix because it goes before the argument, is executed first before other logical operators (higher precednece)
infixr:35 " ∧ " => bin_exp binary_op.and  --binding strength is 35
...
| var_exp v, i => i v --NOTE: applies i to v
| un_exp op e, i => (eval_un_op op) (eval_expr e i)
| bin_exp op e1 e2, i => (eval_bin_op op) (eval_expr e1 i) (eval_expr e2 i) -- use recursion 
