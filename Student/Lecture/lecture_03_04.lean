...
def id_nat : Nat → Nat
| n => n -- NOTES: def is a name, binds a name to a function, takes a Nat as an argument and return a Nat, Nat is binded to n

def id_string : String → String -- NOTE: type of object differs between these three functions
| n => n
...
| α, v => v 
/- NOTES: 
- could do | _, v => v instead
- this is an example of a parametrically polymorphic func., takes a type and a value of that type, can get rid of alpha
- if first argument is Nat, the rest of the funciton is Nat → Nat
-/

-- NOTE: another way of writing this function, main difference is the placement of colons
/- def id_poly (α : Type) : α → α  
| α, v => v -/
...
#eval id_poly Bool Type 
/- NOTES: 
could pass type as an argument but we didn't write the func. like that 
could write id_poly String "Hello!" as (id_poly String) "Hello!" --> the type is String -> String!!!
...
#eval id_poly' Bool true  -- DOESN'T WORK can't give implicit argument ecplicit type unless you put an @
#eval @id_poly' Bool true -- this works!
...
/-NOTES: 
first argument is a function (Nat->Nat), second argument is the parameter
can't take out parentheses for f (f a) becuase functions are left associative
can rewrite this with a circle? 
-/
