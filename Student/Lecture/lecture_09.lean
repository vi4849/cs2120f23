...
-- NOTE: empty type corresponds to false, empty is an uninhabited type
-- any inhabited type is true

--NOTE: function is false (true → false)
-- true → true is true, false → false or true is true
...
def empty_to_empty (e:Empty) : Empty := nomatch e --NOTE: means there are no cases that must be considered

-- NOTES
-- function cannot be called, as you cannot give the function an Empty type
-- function shows that an impossibility creates an impossibility
-- can have a formal parameter for Empty, cannot have an actual parameter

#eval empty_to_empty --NOTE: cannot call this function
...
def empty_to_α (α : Type) : Empty → α    := nomatch e --NOTE: inhabited -> means you can write a function

--NOTE: you can return anything because you can never implement in
--e is an implicit version of false
...
-- NOTE: α must be empty /uninhabited! if α is inhabited, you cannot build a function of that type
...
- Can you define some function, nxe2s : Nat × Empty → String 
 -- Yes? - if Empty is on the left side, then it's fine


- Is the type, Nat × Empty → String, inhabited or not? 
--NOTE: evaluates to false to true, which is true, so it is inhabited

- How many strings can nxe2s possibly return? Why?
--returns zero strings because you cannot actually do #eval

- Can you define a function, noe2s : String ⊕ Empty → Nat 
 -- Yes, function is inhabited

- Is the type, String ⊕ Empty → Nat, inhabited or not? --inhabited
- Can noe2s return any Nat? If so, prove it by example. --can return any Nat?
-/

def test: String ⊕ Empty → Nat
| Sum.inl "one" => 1 
| Sum.inl _ => 2

def test2: String ⊕ Bool → Nat
| Sum.inl a => a.length
| Sum.inr _ => 3

#eval test2 (Sum.inl "he" )
#eval test (Sum.inl "one")


/-
- Is the function type, (Nat → Empty), inhabited or not?
  The function is not inhabited => true → false

- Prove your answer (is (Nat → Empty) uninhabited)
- Is the type, {α : Type} → α → Empty, inhabited or not.
  Sometimes - depends on if α is Empty or not (if it is not, it is inhabited)

- Prove your answer (Is {α : Type} → α → Empty, inhabited?)
- Is the type, Empty → (Nat → Empty) inhabited? Prove it.
  It is uninhabited / false

- Prove this type uninhabited: {α : Type} → α × (α → Empty)

-/

def a2e (α : Type) : (e: α) → Empty := nomatch e

def a1: Empty → Empty := a2e Empty 
def a2: Nat → Empty:= nomatch e
