...
/-
NOTES
- apply2 double is a function that is waiting for an argument (double ∘ double)
- apply2 double 5 equals (apply2 double) 5 (because functions are left associative!)
    - type is Nat → Nat
- apply2 type is Nat → Nat → Nat → Nat 
-/
...
/-
NOTES
if apply glue_funs only to the first two arguments, get a function waiting for alpha to return gamma 
-/
...
def is_even_len := compose' is_even String.length -- returns a function (String → Bool) 
...
def compose {α β γ : Type} :
  ...
  (α → β) → 
  (α → γ)       
| g, f => (fun a => g (f a)) --IMPORTANT
/-
NOTES
- (fun a => g (f a)) is a function (fun = function)
- 
-/
...
-- NOTE: called an anonymous function (writing a function without a name)
...
def is_even_len'' := (is_even ∘ String.length) --NOTE: infix notation in lean for composing function
