...
/-
TYPE OF COMPOSE FUNCTION
- alpha to beta, beta to gamma, alpha to gamma
-/
...
#check @Box.put
#check @Box.put Nat 6 -- have to write Nat because you have @
#check (Box.put 8)
#check @Box.put Nat

/-
NOTES:
- put is a constructor for Box
- type for 7 is Box Nat 
- put is polymorphic (has {} when you do #check)
- put automatically has an implicit argument (can see this in #check)
- get different types out of the box through pattern matching
-/
...
#check (@Box.put)
def jack_in_a_box := @Box.put String "Jack!" --NOTE: type is Box String
...
--NOTE: pattern matching binds name in terms to value (s gets binded to Jack!")
...
#check (Prod.mk 3 true) -- a value (term), type is Prod Nat Bool
...
/-
NOTES
- inl introduce a value on the left, inr introduce a value on the right
- Sum.inl 1 is a type of Sum Nat Bool *****
-/
...
def elim_sum1 : Sum Nat Bool → String --type is bool → string
...
| (Sum.inl a), α2γ, _ => α2γ a  --NOTE: think of Sum.inl as if (Sum left)
